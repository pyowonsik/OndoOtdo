...
//        Log.d("ViewModel Test : MainFragment",weatherViewModel.weatherResponse.value.toString())
    ...
    private fun getOutfitByTemperature(temperature: Int) {
        val outfit: List<String> = when {
            temperature >= 28 -> listOf("민소매", "반팔", "반바지", "원피스")
            temperature in 23..27 -> listOf("반팔", "얇은 셔츠", "반바지", "면바지")
            temperature in 20..22 -> listOf("얇은 가디건", "긴팔", "면바지", "청바지")
